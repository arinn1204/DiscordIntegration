...
public class ChannelRelayChatTrueButMessageEmptyRule implements IConfigurationValidationRule {
    ...
    public ValidationResult validate() {
        Object[] channelIDs = Configuration.getConfig().discord.channels.channels.entrySet().stream()
                .filter(entry -> entry.getValue() != null && entry.getKey() != null)
                .filter(entry -> !entry.getValue().relayChat.isDisabled())
                .filter(entry -> entry.getValue().messages.chatMessage.normal.equals("")).map(Map.Entry::getKey)
                .toArray();

        if (channelIDs.length == 0) {
            return new ValidationResult(true, null);
        } else {
            return new ValidationResult(false, getHint() + " Channel(s): " + Joiner.on(", ").join(channelIDs));
        }
    }
}
