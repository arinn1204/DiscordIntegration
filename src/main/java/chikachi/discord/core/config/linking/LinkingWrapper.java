...
public class LinkingWrapper {
    ...
    public UUID getMinecraftId(long discordId) {
        if (this.linked.containsValue(discordId)) {
            Optional<Map.Entry<UUID, Long>> link = this.linked.entrySet().stream()
                    .filter(uuidLongEntry -> uuidLongEntry.getValue() == discordId).findFirst();
    ...
    public LinkingRequest getRequest(long discordUserId) {
        Optional<LinkingRequest> request = this.requests.stream()
                .filter(linkingRequest -> linkingRequest.getDiscordId() == discordUserId).findFirst();

        return request.orElseGet(() -> LinkingRequest.create(discordUserId));
    }

    public Optional<LinkingRequest> getRequestByCode(String code) {
        return this.requests.stream().filter(linkingRequest -> linkingRequest.getCode().equalsIgnoreCase(code))
                .findFirst();
    }
