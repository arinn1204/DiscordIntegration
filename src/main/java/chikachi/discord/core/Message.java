...
public class Message {
    ...
    WebhookMessage toWebhook(TextChannel channel) {
        return new WebhookMessage(formatText(message.webhook, channel), this.author, this.avatarUrl);
    }
                ...
                while (m.find()) {
                    ...
                    if (Configuration.getConfig().minecraft.dimensions.generic.canMentionUsers) {
                        if (channel instanceof TextChannel textChannel) {
                            Optional<Member> theMember = textChannel.getGuild().getMembersByName(name, true).stream()
                                    .filter(
                                            member -> hash == null
                                                    || member.getUser().getDiscriminator().equalsIgnoreCase(hash))
                                    .filter(member -> member.hasPermission(Permission.MESSAGE_HISTORY)).findAny();
                    ...
                    if (Configuration.getConfig().minecraft.dimensions.generic.canMentionRoles) {
                        if (channel instanceof TextChannel textChannel) {
                            Optional<Role> theRole = textChannel.getGuild().getRolesByName(name, true).stream()
                                    .filter(Role::isMentionable)
                                    .filter(role -> role.hasPermission(Permission.MESSAGE_HISTORY)).findAny();
