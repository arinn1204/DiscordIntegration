...
public class CommandConfig {
    ...
    public boolean shouldExecute(String command, User executor, MessageChannel channel) {
        return isEnabled() && (this.name.equalsIgnoreCase(command) || this.aliases.contains(command.toLowerCase()))
                && this.checkPermission(executor, channel);
    }
    ...
    private boolean checkPermission(User user, MessageChannel channel) {
        ...
        for (String permission : permissions) {
            if (permission.startsWith("role:")) {
                ...
                if (user.getId().equals(permission.substring(5))
                        || (user.getName() + "#" + user.getDiscriminator()).equals(permission.substring(5))) {
                    return true;
                }
            } else {
                if (user.getId().equals(permission)
                        || (user.getName() + "#" + user.getDiscriminator()).equals(permission)) {
                    return true;
                }
            }
        }
